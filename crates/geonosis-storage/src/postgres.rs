...
fn idp_from_row(row: &sqlx::postgres::PgRow) -> Result<IdentityProvider, StorageError> {
    ...
    Ok(IdentityProvider {
        id: id.parse().map_err(|e: geonosis_core::id::IdParseError| StorageError::Backend(e.to_string()))?,
        realm_id: realm_id.parse().map_err(|e: geonosis_core::id::IdParseError| StorageError::Backend(e.to_string()))?,
        alias,
        display_name,
...
fn broker_state_from_row(row: &sqlx::postgres::PgRow) -> Result<BrokerAuthnState, StorageError> {
    ...
    Ok(BrokerAuthnState {
        ...
        return_to_flow_state: return_to.parse().map_err(|e: geonosis_core::id::IdParseError| StorageError::Backend(e.to_string()))?,
        created_at,
        expires_at,
    })
}
...
fn broker_link_from_row(row: &sqlx::postgres::PgRow) -> Result<BrokerLink, StorageError> {
    ...
    Ok(BrokerLink {
        id: id.parse().map_err(|e: geonosis_core::id::IdParseError| StorageError::Backend(e.to_string()))?,
        realm_id: realm_id.parse().map_err(|e: geonosis_core::id::IdParseError| StorageError::Backend(e.to_string()))?,
        user_id: user_id.parse().map_err(|e: geonosis_core::id::IdParseError| StorageError::Backend(e.to_string()))?,
        idp_alias,
        external_id,
        external_username,
        created_at,
