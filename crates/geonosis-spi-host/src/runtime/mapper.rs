    ...
    pub async fn map_claims(
        ...
        let func = instance
            .get_typed_func::<
                ...
                (Result<Vec<u8>, geonosis_plugin_error::Wire>,),
        ...
        let res = tokio::time::timeout(self.limits.wall_clock(), call)
            ...
            .map_err(|e| classify_call_error(e, &self.limits))?;
...
fn classify_call_error(e: anyhow::Error, limits: &ResourceLimits) -> RuntimeError {
    let msg = e.to_string();
...
/// Wire shape of the WIT `plugin-error` record. The field order
/// mirrors `wit/types.wit` so the component-model layout matches.
mod geonosis_plugin_error {
