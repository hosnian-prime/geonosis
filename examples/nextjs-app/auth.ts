...
// matching client config on the Geonosis side is `master-web` from
// the quickstart bootstrap (see deploy/compose/quickstart.yml).

import NextAuth, { type NextAuthConfig } from "next-auth";

const issuer = process.env.GEONOSIS_ISSUER ?? "http://localhost:8080/realms/master";
const clientId = process.env.GEONOSIS_CLIENT_ID ?? "master-web";
